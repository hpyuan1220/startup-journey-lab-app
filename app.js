...
async function save(status){ const data=formData(); const missing=required.filter(k=>!data[k]?.trim()); if(status==='submitted'&&missing.length){message('form-message',`尚未完成：${missing.map(k=>({student_name:'姓名',team_preference:'分組角色',verbatim_complaint:'原句或抱怨',observed_context:'當時的現場',observed_problem:'生活不便',affected_user:'受到影響的人',known_fact:'事實',unverified_assumption:'假設',interview_next_question:'下週訪談問題',expected_learning:'期待',concern:'擔心'})[k]).join('、')}`,true);return;} try{const result=await studentApi({action:'save',token:studentSession.token,submission:{...data,status}}); $('updated-at').textContent=`最後更新：${new Date(result.submission.updated_at).toLocaleString('zh-TW')}`;message('form-message',status==='submitted'?'已正式提交，老師現在可以查看。':'草稿已儲存。');}catch(e){message('form-message',e.message,true);}}
