...
async function api(path, options={}){ const res=await fetch(`${cfg.supabaseUrl}${path}`, {headers:{apikey:cfg.supabaseAnonKey,'Content-Type':'application/json',...(options.headers||{})},...options}); const body=await res.json().catch(()=>({})); if(!res.ok) throw new Error(body.error_description||body.error||'連線失敗。'); return body; }
...
async function loadTeacher(){if(!teacherToken)return; try{const rows=await api('/rest/v1/week1_submissions?select=*&order=updated_at.desc',{headers:{Authorization:`Bearer ${teacherToken}`}});teacherRows=rows;renderTeacher();$('teacher-gate').hidden=true;$('teacher-dashboard').hidden=false;}catch(e){teacherToken='';localStorage.removeItem('sjl-teacher-token');$('teacher-gate').hidden=false;$('teacher-dashboard').hidden=true;message('teacher-message','請使用教師 Email 與密碼登入。');}}
